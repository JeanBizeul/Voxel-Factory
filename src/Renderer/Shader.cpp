...
static std::string getShaderCode(const std::string &path)
{
    ...
    if (!file.is_open())
        throw std::runtime_error("VoxelFactory::Shader: Unable to open shader file '"
            + path + "'");
...
static unsigned int compileShader(const std::string &path, unsigned int shaderType)
{
    ...
    if (!success) {
        ...
        throw std::runtime_error("VoxelFactory::Shader: Unable to compile "
            + path + " : " + std::string(infoLog));
    }
    return shader;
}

VoxelFactory::Shader::Shader(const std::string &vertexPath,
    const std::string &fragmentPath)
{
    ...
    if (!success) {
        ...
        throw std::runtime_error("VoxelFactory::Shader::Shader: Unable to compile the shaderProgram: "
         + std::string(infoLog));
    }
    // We don't need the compiled shaders anymore (they takes space in memory)
    glDeleteShader(vertexShader);
    glDeleteShader(fragmentShader);
    Logger::getInstance().log("Loaded and linked " + vertexPath + " + " + fragmentPath,
        "shader", Logger::INFO);
}

VoxelFactory::Shader::~Shader()
{
    glDeleteProgram(_shaderProgram);
}

void VoxelFactory::Shader::use()
{
...
void VoxelFactory::Shader::set(const std::string &name, bool value) const
{
    glUniform1i(glGetUniformLocation(_shaderProgram, name.c_str()), (int)value);
}

void VoxelFactory::Shader::set(const std::string &name, int value) const
{
    glUniform1i(glGetUniformLocation(_shaderProgram, name.c_str()), value);
}

void VoxelFactory::Shader::set(const std::string &name, float value) const
{
    glUniform1f(glGetUniformLocation(_shaderProgram, name.c_str()), value);
}

void VoxelFactory::Shader::set(const std::string &name, glm::vec2 value) const
{
    glUniform2f(glGetUniformLocation(_shaderProgram, name.c_str()),
        value.x, value.y);
}

void VoxelFactory::Shader::set(const std::string &name, glm::vec3 value) const
{
    glUniform3f(glGetUniformLocation(_shaderProgram, name.c_str()),
        value.x, value.y, value.z);
}

void VoxelFactory::Shader::set(const std::string &name, glm::vec4 value) const
{
    glUniform4f(glGetUniformLocation(_shaderProgram, name.c_str()),
        value.x, value.y, value.z, value.w);
}

void VoxelFactory::Shader::set(const std::string &name, glm::mat2 value) const
{
    glUniformMatrix2fv(glGetUniformLocation(_shaderProgram, name.c_str()),
        1, GL_FALSE, glm::value_ptr(value));
}

void VoxelFactory::Shader::set(const std::string &name, glm::mat3 value) const
{
    glUniformMatrix3fv(glGetUniformLocation(_shaderProgram, name.c_str()),
        1, GL_FALSE, glm::value_ptr(value));
}

void VoxelFactory::Shader::set(const std::string &name, glm::mat4 value) const
{
